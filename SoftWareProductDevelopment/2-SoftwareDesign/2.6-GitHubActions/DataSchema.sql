...
CREATE TABLE users (
    ...
    CONSTRAINT chk_users_name CHECK (
        username SIMILAR TO '[a-zA-Z0-9 ]{3,50}'
    ),
    CONSTRAINT chk_users_income CHECK (
        income_level IN ('Low', 'Medium', 'High')
    )
);
...
CREATE TABLE housing (
    ...
    CONSTRAINT fk_housing_provider FOREIGN KEY (provider_id)
...
CREATE TABLE articles (
    ...
    article_doi VARCHAR(100) UNIQUE,
    publish_date DATE
);
...
CREATE TABLE summaries (
    ...
    quality_score INT,
    CONSTRAINT fk_summaries_article FOREIGN KEY (article_id)
...
CREATE TABLE nutrition_items (
    ...
    calories INT,
    CONSTRAINT fk_nutrition_order FOREIGN KEY (order_id)
